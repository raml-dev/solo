...
6. **Use runes-based stores**: each store is a `.svelte.ts` module in the `src/lib/stores` folder, with a single exported `$state` object as source of truth, and consumers read reactively from that state (`$derived` only in components). Avoid writable/subscribe legacy patterns and effect-based sync loops.
...
7. **Testability**: Keep Svelte components logic-free as much as possible. Write all non-component-related logic in dedicated TypeScript files, so it's easier to write unit tests.
8. **Go Models**: Do not edit Go models under `wailsjs/` because they are generated by Wails during builds. If you need to edit those types, edit the Go code that generates them.
9. **Uncertainty**: If you are unsure where a new file, store, or component belongs, stop and ask before creating it.
