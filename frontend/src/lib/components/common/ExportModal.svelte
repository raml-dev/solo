...
<script lang="ts">
  ...
  let { open = $bindable(), title = "Export", options, onExport, onClose }: Props = $props();

  let selectedId = $state("");

  $effect(() => {
    if (open) selectedId = options[0]?.id ?? "";
  });
</script>

<Modal bind:open {title} size="sm" onclose={onClose}>
  ...
  {#snippet footer()}
    <div class="flex w-full justify-end gap-2">
      ...
      <Button color="primary" onclick={() => onExport(selectedId)}>Export</Button>
    </div>
  {/snippet}
</Modal>
